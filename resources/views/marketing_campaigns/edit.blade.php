                ...
                <div class="card-body">
                    <form action="{{ route('marketing-campaigns.update', $campaign) }}" method="POST">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('type') is-invalid @enderror" 
                                        ...
                                        <option value="promotional" {{ old('type', $campaign->type) == 'promotional' ? 'selected' : '' }}>Promotional</option>
                                        <option value="announcement" {{ old('type', $campaign->type) == 'announcement' ? 'selected' : '' }}>Announcement</option>
                                    </select>
                                    @error('type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="col-md-6">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="include_customers" 
                                               name="target_audience[customers]" value="1" 
                                               {{ old('target_audience.customers', $campaign->target_audience['customers'] ?? false) ? 'checked' : '' }}>
                                ...
                                <div class="col-md-6">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="include_leads" 
                                               name="target_audience[leads]" value="1" 
                                               {{ old('target_audience.leads', $campaign->target_audience['leads'] ?? false) ? 'checked' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <a href="{{ route('marketing-campaigns.show', $campaign) }}" class="btn btn-secondary">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    inputs.forEach(input => {
        input.addEventListener('change', function() {
            localStorage.setItem('campaign_edit_draft', JSON.stringify({
    ...
    form.addEventListener('submit', function() {
        localStorage.removeItem('campaign_edit_draft');
    });
});
</script>
@endpush
